...
한국 웹소설 시장의 예비 작가들은 이야기를 가지고 있지만, 체계적인 다회차 원고로 만들어내지 못한다. 병목은 창의력이 아니라 — 수십 개의 사건을 구조화하고, 40회차 이상에 걸쳐 캐릭터와 플롯 일관성을 유지하며, 회차당 3,000-5,000자의 산문을 생산하는 노동이다. 한국이든 해외든, 시각적 스토리 구조와 AI 장면별 초고 작성을 결합한 도구는 존재하지 않는다.
...
2. **컨텍스트 압축이 서사적 충실도를 유지한다.** AI 생성 요약이 15-20개 장면(Phase 1 범위)에 걸쳐 충분한 디테일을 유지한다. 40회차 이상은 검증 필요.
...
- **REST** + JSON. 리소스가 도메인 엔티티(프로젝트, 장면, 캐릭터, 초고)에 매핑.
- **버전 관리**: URL 경로 (`/api/v1/...`). 호환성 깨지는 변경은 새 버전.
- **페이지네이션**: 커서 기반 (타임라인 장면은 위치 순, 프로젝트는 최근 편집 순).
- **스트리밍**: AI 생성용 SSE 엔드포인트 (`/api/v1/scenes/{id}/generate`). 비스트리밍 작업은 표준 POST.
...
| **timeline** | 장면 CRUD, 트랙 관리, NLE 클립 배치 (start_position + duration), 분기/합류 크로스트랙 연결 | 장면 조작, 드래그앤드롭 |
| **character** | 캐릭터 CRUD, 관계 관리, 캐릭터 카드 | 캐릭터 맵 상호작용 |
| **generation** | 컨텍스트 어셈블리, LLM 프롬프트 구성, provider adapter 오케스트레이션, 스트리밍, 요약 생성 | AI 초고 생성 (핵심 가치) |
| **editor** | 초고 저장, 방향 기반 편집 오케스트레이션, 텍스트 버전 관리 | 에디터 상호작용 |
...
- `editor`는 `timeline`, `generation`에 의존 (장면 데이터 읽기, 재생성 트리거)
...
사용자 입력 (텍스트 또는 파일)
  ...
  -> API: 단일 DB 트랜잭션으로 기록
    ...
    -> track 레코드
    -> scene 레코드 (start_position, duration, 트랙 할당)
    -> character 레코드
    -> relationship 레코드
  -> 응답: 완전한 워크스페이스 상태
```
...
사용자가 장면에서 "초고 생성" 클릭
  -> API: generation 모듈이 컨텍스트 어셈블리
    ...
    -> 읽기: 선행 장면의 압축 요약 (DB/캐시)
    -> 읽기: 다른 트랙의 동시 장면 (타임라인 범위가 겹치는 다른 트랙의 장면) (DB)
    -> 읽기: 다음 장면 제목 + 요약 (DB)
  ...
  -> 스트림 완료 시:
    -> 전체 초고 텍스트를 DB에 저장
    -> 장면 상태를 "AI 초고"으로 업데이트
    -> 비동기: 압축 요약 생성
      -> LLM provider adapter: 요약 요청 (경량 모델)
      -> 요약을 DB + 캐시에 저장
```

일관성: 초고 쓰기는 Strong (단일 행 upsert). 요약 생성은 Eventually consistent (비동기, 논블로킹).

**흐름 3: 방향 기반 편집 (부분 재생성)**

```
사용자가 텍스트 선택 + 방향 입력 ("더 긴장감 있게")
  -> API: 선택 범위 + 방향 + 전체 초고 수신
...
|--------|--------|-----------|--------|------|
| **Neon (PostgreSQL)** | 프로젝트, 장면, 트랙, 연결, 캐릭터, 관계, 초고, 요약, 사용자 | 관계형 — 엔티티 간 풍부한 관계. 워크스페이스 상태의 ACID 트랜잭션. | Strong | 무기한 |
...
| 장면 요약 | Redis + DB (원본) | 1시간 | 해당 장면의 초고 변경 시 | 요약 생성 비용 높음 (LLM 호출). 하위 생성의 재계산 방지. |
...
- 부분 초고 보존: 스트림 중간 중단 시 수신된 내용 저장
...
**전략**: 유닛 중심 피라미드. 도메인 로직(컨텍스트 어셈블리, 프롬프트 구성, 장면 정렬)이 순수하고 광범위하게 유닛 테스트됨.

| 계층 | 대상 | 방법 | 목표 |
|------|------|------|------|
| 유닛 | 도메인 로직: 컨텍스트 어셈블리, 장면 정렬, 프롬프트 템플릿, 검증 | `cargo test` — 순수 함수, 모킹 불필요 (hexagonal 포트가 도메인을 의존성 없이 만듦) | 도메인 모듈 80%+ 커버리지 |
| 통합 | DB 어댑터 (SQLx 쿼리), LLM 게이트웨이 클라이언트, R2 클라이언트 | `cargo test` + Neon 브랜치 + provider adapter 테스트 설정 | 모든 어댑터 경계 |
| E2E | 핵심 여정: 프로젝트 생성 → 자동 구조화 → 초고 생성 → 편집 | Playwright + 스테이징 | 크리티컬 흐름 3개 |
...
| 초고 저장 후 요약 생성 | 저장 응답 차단 가능 | 내부 이벤트 버스로 비동기. 실패 시 다음 접근에서 재시도. |
...
| 리스크 | 영향 | 가능성 | 완화 |
|--------|------|--------|------|
| 15+ 장면에서 컨텍스트 어셈블리 프롬프트가 LLM 컨텍스트 윈도우 초과 | 생성 품질 저하 또는 절삭 | 중간 | 토큰 예산 관리: 컨텍스트 16K 토큰, 가장 오래된 요약부터 절삭. 생성당 총 토큰 모니터링. |
| SSE 스트리밍이 긴 생성 중 연결 끊김 | 불완전한 초고 | 낮음 | Cloud Run이 SSE 네이티브 지원. 300초 타임아웃. 클라이언트 재연결 + 마지막 수신 위치. 서버에 부분 초고 저장. |
...
2. **20+ 장면에서 요약 압축 충실도.** 복합 요약(요약의 요약)이 서사적 디테일을 유지하는가? *필요: 25장면 원고로 테스트, 모순 비율 측정.*
...
- **상태**: Accepted
- **컨텍스트**: 멀티트랙 타임라인(드래그앤드롭, 장면 상태 업데이트, 패널 리사이징)과 스트리밍 텍스트 렌더링에 세밀한 반응성 필요.
- **결정**: TanStack Start (SSR) + SolidJS (반응형 UI), Cloudflare Workers 배포.
- **고려한 대안**:
  - **Next.js (React)**: 기각. React의 가상 DOM 디핑은 타임라인에 불필요한 오버헤드 — 단일 장면 상태 변경 시 해당 장면만 업데이트 필요. SolidJS의 세밀한 반응성이 이를 정밀하게 처리. React 생태계 이점(shadcn/ui, Radix)은 커스텀 디자인 시스템("Ink & Amber")이 있어 비중요.
...
- **상태**: Accepted
- **컨텍스트**: 풍부한 관계(프로젝트, 장면, 트랙, 캐릭터, 관계)의 관계형 모델링 필요. 글로벌 대상.
...
- 우선순위: Config > 현재 장면 > 캐릭터 > 전방 > 병렬 > 서사
- 서사 요약 초과 시 가장 오래된 것부터 절삭 (먼 사건일수록 관련성 낮음)

### 10.4 요약 압축

초고 저장 시 압축 요약을 비동기 생성:
...
**품질 리스크**: 복합 요약은 20+ 장면에서 디테일 손실. Phase 1 완화: 요약의 요약 대신 풀 요약 사용. 토큰 사용량 모니터링. 예산 초과 시 계층형 압축: 최근 5개 장면 풀 요약, 오래된 장면 초압축.
...
| 요약 압축 (200-400자 vs. 3K자 초고) | Phase 1 | 서사 컨텍스트 토큰 5-10배 감소 |
...
- 웹 클라이언트: 대시보드, 프로젝트 생성, 워크스페이스 (Config Bar, 타임라인, 캐릭터 맵, 에디터, 장면 상세)
